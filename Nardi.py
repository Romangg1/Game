...
class GameWindow:
    def __init__(self, root):
        ...
        self.black_chip_positions = [10] * 11
        ...
        self.dice_images = [ImageTk.PhotoImage(Image.open(f"C:/Users/huawei/Documents/Нарды/image/{i + 1}.png").resize((50, 50))) for i in range(6)]
        self.black_chip = ImageTk.PhotoImage(Image.open("C:/Users/huawei/Documents/Нарды/image/black.png").resize((50, 50)))
        self.white_chip = ImageTk.PhotoImage(Image.open("C:/Users/huawei/Documents/Нарды/image/white.png").resize((50, 50)))
        ...
        self.dice_button = tk.Button(self.game_window, text='Бросить кубики', bg='brown', fg='white',
                                     command=self.roll_dice)
        self.dice_button.grid(row=5, column=0)

        self.exit_button = tk.Button(self.game_window, text='Выход', command=self.return_to_main_menu, bg='brown',
                                     fg='white')
        self.exit_button.grid(row=5, column=3)

    def create_score_labels(self):
        self.player1_label = tk.Label(self.game_window, text='Игрок 1', fg='blue', bg='white')
        self.player1_label.grid(row=0, column=0, sticky='nsew')

        self.player1_score = tk.IntVar()
        self.player1_score.set(0)
        self.player1_score_label = tk.Label(self.game_window, textvariable=self.player1_score, bg='white')
        ...
        self.player2_score_label = tk.Label(self.game_window, textvariable=self.player2_score, bg='white')
    ...
    def roll_dice(self):
        ...
        if dice1 == dice2:
            self.turn.set(f'Ход: Игрок {self.current_player}, кубики: {dice1}, {dice2}, {dice2}, {dice2}')
        else:
            self.turn.set(f'Ход: Игрок {self.current_player}, кубики: {dice1}, {dice2}')

    def move_chip(self, event):
        if not self.remaining_moves:
            self.turn.set('Сначала бросьте кубики!')
            return
        ...
        if (self.current_player == 1 and chip_color == 'white') or (self.current_player == 2 and chip_color == 'black'):
            ...
            if chip_color == 'white':
                ...
                if self.white_chip_positions[chip_number] >= 11:
                    self.white_chip_positions[chip_number] = 42  # Фишка достигла последней позиции
                    self.white_chip_in_goal += 1
                    self.player1_score.set(self.white_chip_in_goal)
                    self.game_field.delete(item)  # Удаляем фишку с поля
                ...
                self.black_chip_positions[chip_number] += move - 1
                if self.black_chip_positions[chip_number] <= 0:
                    self.black_chip_positions[chip_number] = 0  # Фишка достигла последней позиции
                    self.black_chip_in_goal += 1
                    self.player2_score.set(self.black_chip_in_goal)
                    self.game_field.delete(item)  # Удаляем фишку с поля
            ...
            if not self.remaining_moves:
                ...
                self.turn.set(
                    f'Ход: Игрок {self.current_player}, осталось ходов: {list(map(abs, self.remaining_moves))}')

    def calculate_new_position(self, position, color):
        if position < 11:  # Нижний ряд
    ...
    def check_win(self):
        if self.white_chip_in_goal == 11:
            messagebox.showinfo("Победа!", "Победил игрок 1!")
            self.return_to_main_menu()

        elif self.black_chip_in_goal == 11:
            messagebox.showinfo("Победа!", "Победил игрок 2!")
            self.return_to_main_menu()
